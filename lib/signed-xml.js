...
SignedXml.prototype.validateSignatureValue = function(doc) {
  var signedInfo = utils.findChilds(this.signatureXmlDoc, "SignedInfo") 
...
SignedXml.prototype.loadSignature = function(signatureXml) {
  if (typeof signatureXml === 'string') {
    this.signatureXmlDoc = signatureXml = new Dom().parseFromString(signatureXml);
  } else {
    this.signatureXmlDoc = signatureXml;
  }

  this.signatureXml = signatureXml.toString();
  
  var nodes = select(signatureXml, "//*[local-name(.)='CanonicalizationMethod']/@Algorithm")
  ...
  this.signatureAlgorithm = 
    utils.findFirst(signatureXml, "//*[local-name(.)='SignatureMethod']/@Algorithm").value

  this.references = []
  var references = select(signatureXml, "//*[local-name(.)='SignedInfo']/*[local-name(.)='Reference']")
  ...
  this.signatureValue = 
    utils.findFirst(signatureXml, "//*[local-name(.)='SignatureValue']/text()").data.replace(/[\r|\n]/g, '')

  this.keyInfo = select(signatureXml, "//*[local-name(.)='KeyInfo']")  
}
...
SignedXml.prototype.computeSignature = function(xml, xpathToNodeBeforeSignature) {  
  ...
  if (xpathToNodeBeforeSignature) {
    ...
    if (!nodes && nodes.length === 0) {
      ...
      // insert signature before referenceNode.nextSibling
      // if referenceNode.nextSibling is undefined, signature will be included at the end of the doc
      doc.documentElement.insertBefore(signatureDoc.documentElement, referenceNode.nextSibling);
    }
  } else {
    doc.documentElement.appendChild(signatureDoc.documentElement)
  }
